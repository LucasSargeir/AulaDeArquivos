...
int main(int argc, char** argv)
{
    ...
    char linha[2048];
	char nis[15];  // nis(indexado na arvore B) eh a chave para busca
	long posicao;
	char* campo;
    FILE *f; 
    ...
    if(argc != 2)
	{
		fprintf(stderr, "Digite o nis por comando de linha: %s [nis]\n", argv[0]);
		return 1;
	}

    strncpy(nis, argv[1], 15); 
    ...
    if(posicao != -1){
        ...
        //printf("O nis está na árvore\n");

    }else{
        printf("O nis não está na árvore\n"); 
    }
