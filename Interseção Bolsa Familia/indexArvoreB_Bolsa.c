...
void ArvoreB_PrintDebug(ArvoreB* arvore)
{
    ...
    while(fread(pagina,sizeof(ArvoreB_Pagina),1,arvore->f))
    {
        ...
        for(i=0; i<pagina->quantidadeElementos; i++)
        {
            ...
            printf("(%.15s)|%5ld|",pagina->elementos[i].chave, pagina->elementos[i].paginaDireita);
        }
        printf("\n");
        posicaoPagina = ftell(arvore->f);
    }
...
{
    // indexação do arquivo de CEP em árvore B
    ...
    if(!f){
        printf("Não foi possivel abrir o arquivo do bolsa familia\n");
        return 1;  
    }

	fgets(linha, 2048, f); // a primeira linha e o cabecalho
	posicao = ftell(f); // guarda a posicao onde de fato comeca o arquivo, tirando a primeira linha que e so o cabecalho
	fgets(linha, 2048, f);

	while(!feof(f)) // enquanto nao for o fim do arquivo
	{
		coluna = 0;
		campo = strtok(linha,"\t");
		while(campo)
		{
			if(coluna == 7)
			{
                ...
                ArvoreB_Insere(a, nis, posicao); // posicao que se encontra o nis no arquivo do bolsa
			}
			coluna++;
			//printf("%s",campo);
			campo = strtok(NULL,"\t");
