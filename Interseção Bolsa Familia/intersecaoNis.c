...
{
//____________Arquivo previamente indexado  

    ArvoreB* a = ArvoreB_Abre("arvore.dat");

    long posicaoIndex;
    FILE *fIndex;
    fIndex = fopen("intersecao.txt","w"); 

//____________Arquivo que vai ser percorrido 
    ...
    while(!feof(f)) 
    {
        ...
        while(campo)
        {
            if(coluna == 7)
            {
                ...
                posicaoIndex = ArvoreB_Busca(a, nis);
                
                if(posicaoIndex != -1 ){
                    
                    //fwrite(nis,sizeof(char)*15,1,fIndex); estava printando em Binário, não entendemos pq?
                    fputs(nis, fIndex);
                    fputs("\n", fIndex);
                    printf("NIS => %s \n", nis);    
